...
def _context_base(user=None):
    """Shared context for create/edit forms."""
    from apps.whatsapp.models import NumeroWhatsApp
    numeros_qs = NumeroWhatsApp.objects.filter(activo=True)
    if user is not None and not user.can_see_all:
        numeros_qs = numeros_qs.filter(agentes=user)
    campos = list(
    ...
    for v in variables_disponibles:
        v['slug'] = v['nombre']
    return {
        'numeros': list(numeros_qs.order_by('orden', 'nombre')),
...
class DifusionCreateView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ctx = _context_base(request.user)
        ctx['total_preview'] = 0
    ...
    def post(self, request):
        ...
        if not request.user.can_see_all:
            numeros_qs = numeros_qs.filter(agentes=request.user)
        numero = numeros_qs.filter(pk=numero_id).first()
        ...
        if not numero:
            messages.error(request, 'Seleccioná desde qué número enviar la difusión.')
            ctx = _context_base(request.user)
            ctx['post_data'] = request.POST
            return render(request, self.template_name, ctx)
