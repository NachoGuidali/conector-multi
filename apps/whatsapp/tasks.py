...
def auto_asignar_agente(conv) -> bool:
    """
    Asigna la conversación al agente activo, asignado al número de la conversación,
    con menos conversaciones abiertas. Solo considera agentes (rol='agente') activos.
    Retorna True si se asignó, False si no hay agentes disponibles.
    """
    from django.contrib.auth import get_user_model
    User = get_user_model()

    # Agentes activos, en turno, asignados a este número, que reciben asignaciones automáticas
    agentes = (
        User.objects
        .filter(rol=User.ROL_AGENTE, is_active=True, en_turno=True, recibe_asignaciones=True,
                numeros=conv.numero_id)
    ...
    if not agentes.exists():
        # Fallback: si no hay nadie en turno, intentar con cualquier agente del número que reciba asignaciones
        agentes = (
            User.objects
            .filter(rol=User.ROL_AGENTE, is_active=True, recibe_asignaciones=True, numeros=conv.numero_id)
            .annotate(carga=Count(
                'conversaciones',
                filter=Q(conversaciones__archivada=False)
            ))
    ...
    if not agentes.exists():
        logger.warning('Auto-asignación: no hay agentes disponibles para el número de conv %s', conv.pk)
        return False
...
def process_incoming_message(self, message_data: dict):
    from .models import Conversacion, Mensaje, NumeroWhatsApp

    phone = message_data.get('from_phone', '')
    numero_id = message_data.get('numero_id')
    if not phone or not numero_id:
        return
    if not phone.startswith('+'):
        phone = '+' + phone

    try:
        numero = NumeroWhatsApp.objects.get(pk=numero_id)
    except NumeroWhatsApp.DoesNotExist:
        logger.warning('process_incoming_message: numero %s no existe', numero_id)
        return
    ...
    try:
        ...
        if msg_type in ('image', 'audio', 'video', 'document', 'sticker') and message_id:
            ...
            local_url = EvolutionClient(conv.numero).download_and_save_media(
                message_id, conv.pk,
...
def send_whatsapp_message_task(self, mensaje_id: int):
    ...
    from .sender import EvolutionClient
    try:
        msg = Mensaje.objects.select_related('conversacion__numero').get(pk=mensaje_id)
        result = EvolutionClient(msg.conversacion.numero).send_text_message(msg.conversacion.telefono, msg.contenido)
