...
def _numeros_disponibles():
    from apps.whatsapp.models import NumeroWhatsApp
    return NumeroWhatsApp.objects.filter(activo=True).order_by('orden', 'nombre')
...
class UserCreateView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {
            'rol_choices': User.ROL_CHOICES, 'data': data, 'numeros': _numeros_disponibles(),
        })
    ...
    def post(self, request):
        ...
        numeros_ids = request.POST.getlist('numeros')

        if not username or not password:
            messages.error(request, 'Usuario y contraseña son requeridos.')
            return render(request, self.template_name, {
                'rol_choices': User.ROL_CHOICES, 'data': request.POST.dict(), 'numeros': _numeros_disponibles(),
            })
...
class UserUpdateView(AdminRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(request, self.template_name, {
            'obj': user, 'rol_choices': User.ROL_CHOICES, 'data': data,
            'numeros': _numeros_disponibles(),
            'numeros_asignados': set(user.numeros.values_list('pk', flat=True)),
        })
    ...
    def post(self, request, pk):
        ...
        user.save()
        user.numeros.set(request.POST.getlist('numeros'))
